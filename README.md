...
A library for working ergonomically with optional values in Roc. The `Maybe` type represents a value that may or may not exist.

Please consider using Roc's sublime [tag union types](https://www.roc-lang.org/tutorial#tag-union-types) or `Result` over `Maybe`. It can be tempting to reach for `Maybe` due to our previous experience but Roc's tags are usually the better choice as they let you model your situation precisely and with more context. Still, here I am writing `Maybe` so that we'll have a great experience working with plain optional values when that's the best fit!
